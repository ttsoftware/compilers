...
(****************************************************)
  ...
  fun pp_pos (p1, p2) : string = "(" ^ Int.toString(p1) ^ ", " ^ Int.toString(p2) ^ ")"
  ...
  fun pp_exps [] = ""
    | pp_exps ( (e::es) : Exp list ) : string = 
        pp_exp e ^ concat (map (fn x => ", " ^ pp_exp x) es)
...
(***********************************************)
 ...
 (* | typeOfExp ( Pow    (a,b,_) ) = BType Int *)
    | typeOfExp ( LValue (Var    (_,t)      , p) ) = t
    | typeOfExp ( LValue (Index ((v,t),inds), p) ) =
        ( case t of
            Array(r,btp) =>
                if (r <> 0) andalso (r - (length inds) = 0) then 
                    BType btp
                else 
                    raise Error("In typeOfExp of Index: illegal var "^v^" of type: "^
                                 pp_type t^" indexed with "^pp_exps inds^", at ", p)
          | tp => 
                raise Error("In typeOfExp of Index: illegal var "^v^
